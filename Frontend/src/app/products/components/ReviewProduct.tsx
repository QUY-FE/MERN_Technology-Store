...
const ReviewProduct = ({ productId }: { productId: string }) => {
  ...
  const { data: orders = [] } = useGetOrderByEmailQuery(emailUser!, {
    skip: !emailUser,
  });
  ...
  const onSubmit: SubmitHandler<IReviewForm> = async (data) => {
    ...
    if (hasReviewed)
      return toast.error("Bạn đã gửi đánh giá cho sản phẩm này.");
    if (!emailUser) return toast.warning("Vui lòng đăng nhập để đánh giá.");
      ...
      <div className="space-y-4 mb-8">
        {reviews.length > 0 ? (
          ...
          <p className="text-gray-400 italic py-4">
            Hãy trải nghiệm sản phẩm và trở thành người đầu tiên đánh giá nào
          </p>
        )}
      </div>
        ...
        hasPurchased ? (
          hasReviewed ? (
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 text-gray-500 italic text-sm">
              Bạn đã trải nghiệm sản phẩm này và đã gửi đánh giá. Cảm ơn ý kiến
              đóng góp của bạn!
            </div>
          ) : (
            <form
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <FaStar
                        size={20}
                        color={star <= currentRating ? "#ffad33" : "#e4e5e9"}
                      />
                    </button>
                  ))}
                </div>
